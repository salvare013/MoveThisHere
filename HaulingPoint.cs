...
#pragma warning restore CS0649
        ...
        [Serialize]
        private float userMaxCapacity = 2f;
        ...
        public void SetSliderValue(float value, int index)
        {
            ...
            if (capacityKg != userMaxCapacity) //setslidervalue runs each time slider appears AND if changed - check if actually changed to avoid unncessary job interruptions
            {
                if (capacityKg > 100f)
                {
                    capacityKg = (float)Math.Round((decimal)capacityKg);
                ...
                userMaxCapacity = capacityKg; //set both local and Storage variable, local variable gets kept on save/load
        ...
        private void ForceDiamondPrimaryElement()
        {
    ...
    public class DeconstructableHaulingPoint : Workable
    {
        ...
        //however it won't drop any resources from the building itself
